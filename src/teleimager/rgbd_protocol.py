...
remain unchanged. This module defines the additive, single-message RGBD stream
used when a consumer needs both images from the same RealSense frameset.
"""
...
class TeleRgbdFrame:
    """Encoded colour and aligned-depth images from one RealSense frameset."""
...
def pack_rgbd_packet(
    ...
    """Pack one colour JPEG and aligned uint16-depth PNG into one message."""
...
def unpack_rgbd_packet(
    ...
    *,
    ...
    ``received_monotonic_ns`` is local receiver metadata and is not part of the
    wire payload. Monotonic timestamps from different hosts are not comparable.
    """
